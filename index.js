...
const MAX_EMPTY_POLL = 3;
let countOfPollsWithoutWork = 0;
...
function maybeContinuePolling() {
  ...
  else if (countOfPollsWithoutWork > MAX_EMPTY_POLL) {
    console.log(`No messages to process after ${MAX_EMPTY_POLL} attempts, exiting.`);
    process.exit(0);
  }
  else {
...
async function processMessage(message) {
  ...
  do {
    ...
    console.log(`Finishing loop`, ++loopCount);
  } while ((Date.now() - startTime) < TEN_MINUTES_IN_MS);
...
async function pollForWork() {
  ...
  if (message) {
    countOfPollsWithoutWork = 0;
    await processMessage(message);
  }
  else countOfPollsWithoutWork++;
